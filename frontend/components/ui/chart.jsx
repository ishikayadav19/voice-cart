...
const ChartTooltipContent = React.forwardRef(({ active, payload, className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey, ...props }, ref) => {
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const value = !labelKey && typeof label === "string" ? config[label]?.label || label : label?.label;
    if (labelFormatter) {
              ...
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  {itemConfig?.icon ? (
                    ...
                    !hideIndicator && (
                      <div className={cn("shrink-0 rounded-[2px] border-[--color-border] bg-[--color-bg]", indicator === "dot" ? "h-2.5 w-2.5" : indicator === "line" ? "w-1" : indicator === "dashed" ? "w-0 border-[1.5px] border-dashed bg-transparent my-0.5" : "")}
...
const ChartLegendContent = React.forwardRef(({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey, ...props }, ref) => {
  ...
  return (
    <div ref={ref} className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)} {...props}>
      {payload.map((item) => {
